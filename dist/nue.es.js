...
const flushBatch = (batch, mutations) => {
  batch.forEach(cb => {
    ...
    if ((_node && _node._isSubscribed) || !_node) {
      cb(mutations);
      if (process.env.NODE_ENV !== 'production' && _node) {
        nodeUpdated(_node);
      }
    }
  });
...
const subscribe = (baseComp, statePath, updateCb, batch) => {
  ...
  if (process.env.NODE_ENV !== 'production' && !originComp) {
...
const syncNode = (comp, node, deps, update) => {
  ...
  const subscriber = () => {
    ...
    if (process.env.NODE_ENV !== 'production') nodeUpdated(node);
...
const addHooks = (comp) => {
  ...
  comp.hooks = {
    ...
    onMutate: (cb, slices) => {
      if (process.env.NODE_ENV !== 'production' && !slices.length) {
...
const hydrateProp = (target, attribute, comp) => {
  ...
  if (target.matches('input, textarea')) {
    ...
    if (process.env.NODE_ENV !== 'production' && _type === placeholderTypes._functional) {
...
const getNewState = (loopInfo) => {
  ...
  if (process.env.NODE_ENV !== 'production') checkUniquenessOfKeys(keys, _parentComp);
...
const parseAttributes = (element, compName, comp) => {
  ...
  if (process.env.NODE_ENV !== 'production' && !compName) {
    // DEV ONLy
...
const split = (comp, text) => {
  ...
  if (collectedString) {
    if (process.env.NODE_ENV !== 'production' && collectingVar) {
